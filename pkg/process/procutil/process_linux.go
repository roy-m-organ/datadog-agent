...
func WithBootTimeRefreshInterval(bootTimeRefreshInterval time.Duration) Option {
	return func(p Probe) {
		if linProbe, ok := p.(*probe); ok {
			linProbe.bootTimeRefreshInterval = bootTimeRefreshInterval
		}
	}
}
...
func (p *probe) SetReturnZeroPermStats(enabled bool) {
	p.returnZeroPermStats = enabled
}
func (p *probe) SetWithPermission(enabled bool) {
